...
export default function InvoiceGenerator({ order, onClose }: InvoiceGeneratorProps) {
  ...
  const calculateTotal = () => {
    return formData.items.reduce((sum, item) => sum + (item.quantity * (item.unitPrice || item.rate)), 0);
  };
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      const invoiceItems: InvoiceItem[] = formData.items.map(item => ({
        ...
        amount: item.quantity * (item.unitPrice || item.rate),
        total: item.quantity * (item.unitPrice || item.rate)
      }));
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                <div className="flex-1">
                  ...
                  <span className="text-muted-foreground ml-2">
                    {item.quantity} × ₹{(item.unitPrice || item.rate)} = ₹{(item.quantity * (item.unitPrice || item.rate)).toFixed(2)}
                  </span>
                </div>
                <Button
                  variant="ghost"
