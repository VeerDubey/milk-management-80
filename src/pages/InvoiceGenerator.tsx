...
export default function InvoiceGenerator() {
  const { customers, products, orders, addOrder, addInvoice } = useData();
  ...
  const handleShowPreview = async () => {
    ...
    const invoiceData = {
      ...
      tax: (selectedItems.reduce((sum, item) => sum + item.amount, 0) * taxRate) / 100,
      taxRate: taxRate,
  ...
  const generateInvoice = () => {
    ...
    try {
      ...
      // Create invoice object with proper InvoiceItem structure
      const invoice = {
        id: `INV-${Date.now()}`,
        customerId: selectedCustomerId,
        customerName: customer.name,
