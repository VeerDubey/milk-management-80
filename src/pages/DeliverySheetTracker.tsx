...
export default function DeliverySheetTracker() {
  const { customers, products, vehicles, salesmen, saveDeliverySheetData } = useData();
  ...
  const saveDeliverySheet = () => {
    ...
    const deliverySheetData: DeliverySheetData = {
      date: deliveryDate,
      area: selectedArea,
      items: rows.filter(row => row.customerId).map(row => ({
        customerName: row.customerName,
        area: selectedArea,
        GGH: Number((row.quantities as Record<string, string>)['GGH']) || 0,
        GGH450: Number((row.quantities as Record<string, string>)['GGH450']) || 0,
        GTSF: Number((row.quantities as Record<string, string>)['GTSF']) || 0,
        GSD1KG: Number((row.quantities as Record<string, string>)['GSD1KG']) || 0,
        GPC: Number((row.quantities as Record<string, string>)['GPC']) || 0,
        FL: Number((row.quantities as Record<string, string>)['F&L']) || 0,
        totalQty: row.total,
        amount: row.amount
      })),
      totals: {
        GGH: rows.reduce((sum, row) => sum + (Number((row.quantities as Record<string, string>)['GGH']) || 0), 0),
        GGH450: rows.reduce((sum, row) => sum + (Number((row.quantities as Record<string, string>)['GGH450']) || 0), 0),
        GTSF: rows.reduce((sum, row) => sum + (Number((row.quantities as Record<string, string>)['GTSF']) || 0), 0),
        GSD1KG: rows.reduce((sum, row) => sum + (Number((row.quantities as Record<string, string>)['GSD1KG']) || 0), 0),
        GPC: rows.reduce((sum, row) => sum + (Number((row.quantities as Record<string, string>)['GPC']) || 0), 0),
        FL: rows.reduce((sum, row) => sum + (Number((row.quantities as Record<string, string>)['F&L']) || 0), 0),
        QTY: rows.reduce((sum, row) => sum + row.total, 0),
        AMOUNT: rows.reduce((sum, row) => sum + row.amount, 0)
      }
    };

    saveDeliverySheetData(deliverySheetData);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-300">Create and manage daily product delivery sheets</p>
          </div>
