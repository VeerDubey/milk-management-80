...
export class DeliverySheetService {
  ...
  static generateDeliverySheet(
    ...
    // Calculate total amount for the entire track sheet
    const trackSheetTotalAmount = rows.reduce((sum, row) => sum + row.amount, 0);
    ...
    const trackSheet: TrackSheet = {
      ...
      salesmanName: '',
      area: area || '',
      orders: filteredOrders.map(order => order.id),
      totalAmount: trackSheetTotalAmount,
      routeName: area,
      status: 'pending',
